...
public enum HermesHTTPError: LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .unauthorized:
            return "Invalid API key — set hermes.api_key in ~/.config/agent-meow/config.json to match API_SERVER_KEY"
        case .payloadTooLarge:
            return "Screenshot too large for Hermes (10 MB limit) — retry after rebuild with latest Agent Meow"
