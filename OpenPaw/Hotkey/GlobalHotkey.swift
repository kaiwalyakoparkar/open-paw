...
private func openPawHotKeyHandler(
...
final class GlobalHotkey {
    ...
    func register() {
        guard let key = HotkeyKeyMap.virtualKeyCode(for: combo.key) else {
            NSLog("open-paw: unknown hotkey key %@", combo.key)
            return
        }
        let mods = HotkeyKeyMap.carbonModifiers(from: combo.modifiers)
        var spec = EventTypeSpec(eventClass: OSType(kEventClassKeyboard), eventKind: UInt32(kEventHotKeyPressed))
        InstallEventHandler(GetApplicationEventTarget(), openPawHotKeyHandler, 1, &spec, nil, &handler)
